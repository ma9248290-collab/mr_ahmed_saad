       ...
       // ==========================================
        ...
        // ==========================================
        let globalTeacherId = "AhmedSaad_System"; // 🔒 التثبيت هنا
        ...
        async function loadTeacherInfoInitial() {
            try {
                ...
                if (settings) {
                    let tName = settings.teacherName || "أحمد سعد";
                    let cName = settings.centerName || "El-Senior";

                    // أسماء المستر في الصفحة الرئيسية والمودال والمنصة
                   if(document.getElementById("nav-brand-name")) document.getElementById("nav-brand-name").innerText ="الفيزيائى: أحمد سعد"; // مؤقت لحد ما الطالب يسجل
        ...
        window.onload = function() {
            const savedCode = localStorage.getItem("ahmedsaad_student_code");
            const savedPhone = localStorage.getItem("ahmedsaad_parent_phone");
            ...
            try {
                ...
                if (licData) {
                    ...
                    if (licData.status === 'suspended' || isExpired) {
                        ...
                        if(isAutoLogin) {
                            localStorage.removeItem("ahmedsaad_student_code"); localStorage.removeItem("ahmedsaad_parent_phone"); localStorage.removeItem("ahmedsaad_teacher_id");
                        }
                        return; 
                    }
                }
                ...
                localStorage.setItem("ahmedsaad_student_code", code);
                localStorage.setItem("ahmedsaad_parent_phone", phone);
                localStorage.setItem("ahmedsaad_teacher_id", globalTeacherId);
        ...
        function logoutStudent() {
            localStorage.removeItem("ahmedsaad_student_code");
            localStorage.removeItem("ahmedsaad_parent_phone");
            localStorage.removeItem("ahmedsaad_teacher_id");
            location.reload(); 
        }
        ...
        async function submitRegistration() {
            globalTeacherId = "AhmedSaad_System";
            let type = document.getElementById("regType").value;
        ...
        async function retrieveStudentCode() {
            ...
            try {
                let globalTeacherId = "AhmedSaad_System";
                let res = await fetch(`https://el-senior-system-default-rtdb.europe-west1.firebasedatabase.app/${globalTeacherId}/data/students.json`);
                let studentsArray = await res.json() || [];
                studentsArray = Array.isArray(studentsArray) ? studentsArray : Object.values(studentsArray).filter(s => s !== null);
...
// تأثير الآلة الكاتبة
const texts = ["الفيزياء", "القوانين", "نواتج التعلم", "حل الامتحانات"];
let count = 0;
let index = 0;
let currentText = '';
let letter = '';
